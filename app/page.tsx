...
export default function JaramVillageDemo() {
  ...
  return (
    <main className="demo-stage">
      <section className="phone-shell" aria-label="자람마을 모바일 데모">
        ...
        <div className="app-shell">
          <div className="app-scroll" id="app-scroll">
            {route === 'home' && <HomeScreen filter={filter} query={query} products={filteredProducts} addedListing={addedListing} onFilter={setFilter} onQuery={setQuery} onOpen={(product) => { setSelectedProduct(product); navigate('detail'); }} />}
      ...
      <noscript>이 데모의 화면 이동을 보려면 JavaScript를 켜주세요. 홈 화면의 콘텐츠는 자람마을 시연용 예시입니다.</noscript>
    </main>
  );
}
...
function HomeScreen({ filter, query, products, addedListing, onFilter, onQuery, onOpen }: { filter: string; query: string; products: Product[]; addedListing: boolean; onFilter: (value: string) => void; onQuery: (value: string) => void; onOpen: (product: Product) => void }) {
  return <div className="screen home-screen"><BrandHeader /><div className="screen-content"><div className="search-wrap"><Search aria-hidden="true" /><Input value={query} onChange={(event) => onQuery(event.target.value)} placeholder="다음 사이즈나 성장꾸러미를 찾아보세요" aria-label="성장꾸러미 검색" /></div><div className="category-row" aria-label="카테고리">{categories.map((category) => <button key={category} type="button" className={filter === category ? 'active' : ''} onClick={() => onFilter(category)}>{category}</button>)}</div><section className="cycle-banner"><div><span className="eyebrow">성장순환 가이드</span><h1>작아진 옷은 보내고<br />다음 옷을 맞이해요</h1><p>사진 한 번으로 성장꾸러미를 준비해보세요.</p></div><img src="/jarammaeul-symbol.png" alt="" /></section><div className="section-heading"><div><span>우리 동네 성장꾸러미</span><strong>{filter === '전체' ? '지금 필요한 다음 옷을 찾아보세요' : `${filter} 조건으로 모았어요`}</strong></div><button type="button">최신순</button></div><div className="product-grid">{addedListing && <ProductCard product={newListingProduct} onOpen={() => onOpen(newListingProduct)} highlight />}{products.map((product) => <ProductCard key={product.id} product={product} onOpen={() => onOpen(product)} />)}</div>{products.length === 0 && <div className="empty-state"><Search /><strong>조건에 맞는 성장꾸러미가 없어요</strong><span>다음 옷 기다림을 등록하면 알려드릴게요.</span></div>}</div></div>;
}
...
function SellScreen({ onBack, onReview }: { onBack: () => void; onReview: () => void }) {
  ...
  return <div className="screen sell-screen"><ScreenTop title="성장꾸러미 등록" onBack={onBack} /><div className="screen-content compact-content"><div className="demo-label"><Sparkles /> 시연용 예시 · 실제 AI 호출 없음</div><div className="step-heading"><span>1</span><div><strong>사진을 한 번에 올려주세요</strong><small>같은 계절·사이즈 옷을 6장 선택했어요.</small></div></div><div className="sample-grid">{samples.map((src, index) => <div key={src}><img src={src} alt={`아동복 샘플 ${index + 1}`} /><span>{index + 1}</span></div>)}</div><div className="step-heading"><span>2</span><div><strong>AI가 사진을 정리했어요</strong><small>틀린 정보만 확인하면 돼요.</small></div></div><div className="ai-result-card"><div className="ai-result-head"><Sparkles /><strong>110호 가을 성장꾸러미</strong><Badge>92% 확신</Badge></div><div className="result-tags"><span>상의 5벌</span><span>바지 1벌</span><span>가을</span><span>상태 양호</span></div><div className="check-row"><Check /><span>라벨 5장 확인</span><small>가디건 1벌은 직접 확인</small></div></div><Button className="brand-button full-button" onClick={onReview}>성장꾸러미 판매글 확인</Button></div></div>;
}

function ReviewScreen({ onBack, onComplete }: { onBack: () => void; onComplete: () => void }) {
  return <div className="screen review-screen"><ScreenTop title="판매글 확인" onBack={onBack} /><div className="screen-content compact-content"><div className="review-cover"><img src="/product-1.jpeg" alt="110호 가을 성장꾸러미" /><div><Badge>AI 제안</Badge><strong>110호 가을 상의 6벌 성장꾸러미</strong><span>침산동 · 직거래</span></div></div><ReviewSection guardian={false} /><ReviewSection guardian /><Button className="brand-button full-button" onClick={onComplete}>이 내용으로 등록 완료</Button></div></div>;
}

function ReviewSection({ guardian }: { guardian: boolean }) {
  return <section className={`review-section ${guardian ? 'guardian' : ''}`}><div><span>{guardian ? '보호자가 확인한 정보' : 'AI가 정리한 정보'}</span>{guardian ? <Badge variant="secondary"><Check /> 확인 완료</Badge> : <button type="button">수정</button>}</div><dl>{guardian ? <><dt>가격범위</dt><dd>26,000~32,000원</dd><dt>판매가격</dt><dd><strong>29,000원</strong></dd><dt>생활권</dt><dd>대구 북구 침산동</dd></> : <><dt>구성</dt><dd>니트 2 · 맨투맨 2 · 가디건 1 · 바지 1</dd><dt>계절</dt><dd>가을~초겨울</dd><dt>상태</dt><dd>양호 · 미세 사용감</dd></>}</dl></section>;
}
